    ...
    if (isset($_POST['eSbyS'])) {
        ...
        if (empty($name) || empty($gender) || empty($email) || empty($dob) || empty($phone) || empty($address)) {
            ...
            if (mysqli_num_rows($R)) {
                ...
                $R = mysqli_query($conn, $che);
                if ($R) {
                    echo "<script>window.open('../pages/studentProfile.php?update=record has been updated','_self')</script>";
                } else {
...
<body>
    ...
    <div class="cont">
        ...
        <form action="editSbyS.php?id=<?= $idd; ?>" method="post">
            ...
            <button name="eSbyS" class="green-button">Submit</button> <br>
