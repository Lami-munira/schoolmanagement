...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['editsub'])) {
        ...
        $sub = $_POST['name'];
        $che = "Select * from subject where name = '$sub' and id<>'$id';";
        $R = mysqli_query($conn, $che);
        if (mysqli_num_rows($R)) {
            echo '<div class="alert alert-danger" role="alert">Subject already Exist!!</div>';
        } else if (empty($sub)) {
            echo '<div class="alert alert-danger" role="alert">Enter Subject Name!!</div>';
        } else {
            ...
            if ($R) {
                ...
                $R = mysqli_query($conn, $che);
                if ($R) {
                    echo "<script>window.open('../pages/manageSubject.php?add=edited subject','_self')</script>";
                }
            } else {
