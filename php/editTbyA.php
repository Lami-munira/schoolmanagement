...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['back'])) {
        header('Location: ../pages/manageTeachers.php');
    }
    if (isset($_POST['eTbyA'])) {
        ...
        if (empty($name) || empty($gender) || empty($email) || empty($dob) || empty($phone) || empty($address)) {
            ...
            if (mysqli_num_rows($R)) {
                ...
                $R = mysqli_query($conn, $che);
                if ($R) {
                    echo "<script>window.open('../pages/manageTeachers.php?update=teacher record has been updated','_self')</script>";
                } else {
...
<body>
    ...
    <div class="cont">
        <br>
        <h4> The informations of the teacher <br><br></h4>

        <form action="editTbyA.php?id=<?= $idd; ?>" method="post">
            ...
            <button name="eTbyA" class="green-button">Submit</button> <br>
