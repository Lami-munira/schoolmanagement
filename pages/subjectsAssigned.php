...
<body>
    ...
    <center>
        <table>
            ...
            $tid = $_SESSION['Tid'];
            $assigned = "SELECT * FROM subjectsinclass sc join subject sub on sc.subjectID = sub.id where  sub.subjectTeacher = '$tid';";
            $rex = mysqli_query($conn, $assigned);
            if (mysqli_num_rows($rex) == 0) {
            ?>
                <tr>
                    <td colspan="20"> No subjects have been assigned yet </td>
                </tr>
                <?php
            } else {
                while ($lam = mysqli_fetch_array($rex)) {
                    ...
                    $name = $lam[4]; ?>
                    <tr>
                        <th colspan="5"> <?php echo  $name; ?> </th>
                        <th colspan="5"> <?php echo  $cid; ?> </th>
                        <th colspan="10"><a href="grade.php?id=<?= $id;?>">
